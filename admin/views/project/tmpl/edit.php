	...
	<fieldset class="adminform">
		...
		<tr>
			...
			<td style="text-align: left; border: 1px solid #CCCCCC; padding: 4px; width: 300px;">
				<div id='sortable-images' class="manager">
				<?php
...
<script type="text/javascript" src="<?php echo JURI::root();?>components/com_amcportfolio/assets/imageSorter.js"></script>
...
jInsertEditorText = function( text, editor ) {
	switch(editor) {
		...
		case 'project_images' :
			...
			var imgContainer = new Element('div', {'class':'imgOutline'});
			...
			deleteLink.addEvent('click', function(e) {
				new Event(e).stop();
				amcPortImageSorter.removeImage(deleteLink.getParent().getParent());
			});
	...
	$$('a.delete-item').each(function(el) {
		...
		el.addEvent('click', function(e) {
			new Event(e).stop();
			amcPortImageSorter.removeImage(el.getParent().getParent());
		});
	});
